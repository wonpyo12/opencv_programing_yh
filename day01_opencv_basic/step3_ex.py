...
def draw_circle(event,x,y,flags,param):
    global ix,iy,drawing,mode,img,temp_img,mainimg,fx, fy
...
while(1):
    ...
    k = cv.waitKey(1) & 0xFF
    if k == ord('s'):
       if fx != -1 and fy != -1 and ix != fx and iy != fy:
           x1, x2 = min(ix, fx), max(ix, fx)
           y1, y2 = min(iy, fy), max(iy, fy)
           cropped_face = mainimg[y1:y2, x1:x2]
           cv.imwrite("cropped_face.png", cropped_face)
    elif k == ord('q'):
